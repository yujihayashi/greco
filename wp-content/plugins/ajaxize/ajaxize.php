...
/*
    ...
    1.4 : Removed deprecated split function
          Added an option to remove Ajax referer protection (not recommended, but helpful in some rare cases)
...
function ajaxize_this_execute_function($request_str) {
    ...
    list($prefix, $func_name, $signature) = explode(':', $request_str, 3);
...
function ajaxize_request_handler() {
    if ( isset($_REQUEST['ajaxize_this']) ) {
        ...
        if ($options['ajax_referer_check'] == 1) {
